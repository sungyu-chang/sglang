#!/usr/bin/env python3
"""Draw comparison figures from multiple DeepGEMM MoE benchmark runs."""
...
def save_metric_plot(
    ...
    *,
    ...
    ax.set_title("DeepGEMM single expert")
...
def write_summary(output_dir: Path, run_dirs: list[Path], rows: list[dict[str, str]]) -> None:
    metadata = {
        ...
        "num_rows": len(rows),
        "series": sorted({series_label(row) for row in rows}),
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("run_dirs", nargs="+", type=Path, help="Benchmark run directories to compare.")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=None,
        help="Directory for comparison figures. Defaults to results/deepgemm_moe/compare_<timestamp>.",
    )
...
def main() -> int:
    ...
    if output_dir is None:
        output_dir = Path("results") / "deepgemm_moe" / f"compare_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"
