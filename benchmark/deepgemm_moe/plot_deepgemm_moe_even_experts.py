#!/usr/bin/env python3
"""Draw even-split DeepGEMM MoE comparison figures for any expert count."""
...
def load_rows(run_dir: Path) -> list[dict[str, str]]:
    ...
    for row in rows:
        if row.get("distribution") != "even":
            raise ValueError(
                f"Expected even-split rows in {metrics_csv}, got distribution={row.get('distribution')}."
            )
        row["run_dir"] = str(run_dir)
    return rows


def infer_active_experts(rows: list[dict[str, str]], expected_active_experts: int | None) -> int:
    active_experts = {int(row["active_experts"]) for row in rows}
    if len(active_experts) != 1:
        raise ValueError(f"Expected one active expert count, got {sorted(active_experts)}.")
    active_expert_count = next(iter(active_experts))
    if expected_active_experts is not None and active_expert_count != expected_active_experts:
        raise ValueError(
            f"Expected active_experts={expected_active_experts}, got active_experts={active_expert_count}."
        )
...
def title_for_experts(active_experts: int) -> str:
    suffix = "expert" if active_experts == 1 else "experts"
    return f"DeepGEMM {active_experts} {suffix} even split"
...
def save_metric_plot(
    ...
    *,
    ...
    ax.set_title(title_for_experts(active_experts))
...
def write_summary(output_dir: Path, run_dirs: list[Path], rows: list[dict[str, str]], active_experts: int) -> None:
    metadata = {
        "created_at": datetime.now().isoformat(timespec="seconds"),
        "experiment": "even_split",
    ...
    lines = [f"# DeepGEMM {active_experts}-Expert Even-Split Comparison", "", "## Runs", ""]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("run_dirs", nargs="+", type=Path, help="Even-split run dirs to compare.")
    parser.add_argument("--active-experts", type=int, default=None, help="Optional active expert count to validate.")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=None,
        help="Directory for comparison figures. Defaults to results/deepgemm_moe/compare_even_experts_<timestamp>.",
    )
...
def main() -> int:
    ...
    if output_dir is None:
        output_dir = Path("results") / "deepgemm_moe" / f"compare_even_experts_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"
    ...
    active_experts = infer_active_experts(rows, args.active_experts)
    grouped = group_rows(rows)
    write_combined_csv(output_dir, rows)
    write_summary(output_dir, run_dirs, rows, active_experts)
    save_metric_plot(
        output_dir,
        grouped,
        active_experts=active_experts,
    ...
    print(f"Even-split comparison figures written to {output_dir}")
    return 0
